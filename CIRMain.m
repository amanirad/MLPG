function [maxnrm,v,xs,error,xls,res,time,timerror,timeres,ao,mem]= ...
      CIRMain(a_p,b_p,sigma_p,T,x0,start,ep,M,N,Ns,rg,nodetype,method,Nls)
%
% Test case: 
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@comment: What is the meaning of above sentence (each grid)
...
for ell=1:L
  ...
  %
  %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@comment: In AdaptivePts(), range is[0,4] only
...
Nb = 2;
%
 %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ "The evaluation points are always uniform Ns should be larger than N". Why?
%
...
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ In "BDF2coeffs()" I have some questions
%
...
for step = 1:M
  ...
  %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ CIRrhs produce the right hand of the final system? but all component of this vector are zero.!!! 
  %
  ...
  for ell = 1:L
    ...
    if (step==1)
      ...
      if (ls)
        ...
        %@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ In "CIRInitial", I think that you used the exact solution. Why?
      end  
    ...
    if (~ls)
      ...
      for m=1:L
        f = f + BDF2rhs(step,beta1,beta2,Ael{ell,m},Aem{ell,m}, ...
                        lambda_1{m},lambda_2{m},mu_1{m},mu_2{m});
%@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ I don't know what is BDF2rhs, I think it is for time stepping    
      end 
      for m=1:ell-1
