...
a_val = linspace(3,7,15);
sigma_val = linspace(0.05, 0.25, 20);


% Sweep over a and sigma
for i=1:15
    for j=1:20
    Surface2(i,j) = CalculatePDF([a_val(i), 0.05, sigma_val(j)], xi, xb, xls, t1, x0, x_c, data, N_data, eps_c)
    Xval2(i,j) = a_val(i)
    Yval2(i,j) = sigma_val(j)
    IJ = [i,j]
    end
end
